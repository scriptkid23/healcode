...
class AIService:
    ...
    def __init__(self, 
        ...
        # Type assertion to help type checker understand primary_llm is not None
...
"""
    ...
    async def debug_and_fix_with_context(self, error_input: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logging.info(f"Loaded {context_files_count} context files for analysis")
            logging.info(f"Context processing time: {enhanced_context.processing_time_ms}ms")
            logging.info(f"Cache hit: {enhanced_context.cache_hit}")
    ...
    async def get_function_usage_analysis(self, function_name: str, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Log usage analysis information
            logging.info(f"Function '{function_name}' analysis: Found {len(usage_contexts)} usage contexts")
            unique_files = list(set([u.file_path for u in usage_contexts]))
            logging.info(f"Function usage found in {len(unique_files)} different files")
            ...
            logging.info(f"Found {len(similar_functions)} similar functions")
            ...
            return {
                ...
                "similar_functions": [
                    {
                        "file": sf.get("file_name", ""),
                        "language": sf.get("language", ""),
                        "score": sf.get("relevance_score", 0)
    ...
    async def chat(self, user_message: str) -> str:
        ...
        assert self.primary_llm is not None  # Type checker hint
    ...
    async def analyze_codebase_patterns(self, 
        ...
        try:
            ...
            logging.info(f"Starting codebase pattern analysis with max_files={max_files}")
            ...
            for pattern in error_patterns:
                try:
                    ...
                    unique_files = list(set([r.get("FileName", "") for r in results]))
                    pattern_results[pattern] = {
                        "occurrences": len(results),
                        "files": unique_files[:10]
                    }
                    total_files_analyzed += len(unique_files)
                    logging.info(f"Pattern '{pattern}': {len(results)} occurrences in {len(unique_files)} files")
                except:
                    pattern_results[pattern] = {"occurrences": 0, "files": []}
            
            logging.info(f"Codebase analysis completed. Total files analyzed: {total_files_analyzed}")
...
async def create_ai_service(tenant_id: str, 
                           redis_url: str, 
                           model_configs: Dict[str, Dict[str, Any]], 
                           **kwargs) -> AIService:
