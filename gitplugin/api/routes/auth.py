...
@router.post("/logout")
async def logout(
    token: str, security_manager: SecurityManager = Depends(get_security_manager)
):
...
async def get_user_permissions(
    ...
    try:
        ...
        user_perms = security_manager.authz_manager.user_permissions.get(
            current_user.id, []
        )
...
async def list_sessions(
    ...
    try:
        ...
        return {"user_id": current_user.id, "active_sessions": user_sessions}
...
async def revoke_session(
    ...
    try:
        ...
        if session_data[
            "username"
        ] != current_user.username and not security_manager.authz_manager.authorize(
            current_user, admin_permission
        ):
            raise HTTPException(
                status_code=403, detail="Cannot revoke other user's session"
            )
