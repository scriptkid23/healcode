...
class WorkspaceManager:
    ...
    def __init__(
        self,
        base_workspace_dir: str = "/tmp/git_plugin_workspaces",
    ...
    async def allocate_workspace(
        self, job_id: str, workspace_id: Optional[str] = None
    ) -> str:
    ...
    async def get_workspace_stats(self) -> Dict[str, Any]:
        ...
        for workspace in self.workspaces.values():
            ...
            if (
                oldest_workspace is None
                or workspace.created_at < oldest_workspace.created_at
            ):
                oldest_workspace = workspace

            if (
                newest_workspace is None
                or workspace.created_at > newest_workspace.created_at
            ):
        ...
        return {
            ...
            "usage_percentage": round(
                (total_size / self.max_total_size_bytes) * 100, 2
            ),
            "oldest_workspace": oldest_workspace.to_dict()
            if oldest_workspace
            else None,
            "newest_workspace": newest_workspace.to_dict()
            if newest_workspace
            else None,
        }
            ...
            if total_size > self.max_total_size_bytes:
                ...
                for workspace_id, workspace in sorted_workspaces:
                    ...
                    if workspace_id not in workspaces_to_cleanup:
                        ...
                        total_size -= workspace.size_bytes
                        self.logger.info(
                            f"Workspace {workspace_id} marked for cleanup: size limit"
                        )
    ...
    async def create_temp_file(
        self, workspace_id: str, content: str, filename: Optional[str] = None
    ) -> str:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to create temp file in workspace {workspace_id}: {e}"
            )
    ...
    async def read_file(self, workspace_id: str, filename: str) -> str:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to read file {filename} from workspace {workspace_id}: {e}"
            )
    ...
    async def list_files(
        ...
        try:
            ...
            if workspace.base_path.exists():
                if pattern:
                    files = [
                        str(p.relative_to(workspace.base_path))
                        for p in workspace.base_path.rglob(pattern)
                    ]
                else:
                    files = [
                        str(p.relative_to(workspace.base_path))
                        for p in workspace.base_path.rglob("*")
                        if p.is_file()
                    ]
    ...
    async def copy_file(
        self, source_workspace_id: str, dest_workspace_id: str, filename: str
    ) -> str:
        ...
        if not dest_workspace:
            raise WorkspaceException(
                f"Destination workspace not found: {dest_workspace_id}"
            )
