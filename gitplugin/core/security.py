...
class User:
    ...
    def __init__(
        self, user_id: str, username: str, email: str, roles: List[str] = None
    ):
        self.id = user_id
...
class AuthenticationManager:
    ...
    def create_user(
        self, username: str, password: str, email: str, roles: List[str] = None
    ) -> User:
        """Create a new user"""
        ...
        self.users[username] = {"user": user, "password_hash": password_hash}
...
class AuthorizationManager:
    ...
    def _setup_default_roles(self):
        ...
        self.role_permissions["admin"] = [Permission("*", "*")]
    ...
    def authorize(self, user: User, required_permission: Permission) -> bool:
        ...
        self.logger.warning(
            f"Authorization denied for user {user.username}: {required_permission}"
        )
        return False

    def _has_permission(
        self, permissions: List[Permission], required: Permission
    ) -> bool:
...
class AuditLogger:
    ...
    def log_authorization(
        self, username: str, resource: str, action: str, success: bool
    ):
        ...
        try:
            ...
            if not self.authz_manager.authorize(user, required_permission):
                ...
                raise UnauthorizedException(
                    f"Insufficient permissions: {required_permission}"
                )
